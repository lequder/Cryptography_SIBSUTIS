...
void append_padding_bits() {
    ending = static_cast<uint8_t*>(input) + saved_length;
    *ending = 0x80;
    ++ending;

    while ((ending - static_cast<uint8_t*>(input)) % 64 != 56)
        *ending = 0x00, ++ending;
}

void append_length() {
    uint64_t length = saved_length * 8;
    memcpy(static_cast<void*>(ending), &length, 8);

    if (endian() == BIG_ENDIAN) {
        std::swap(*ending, *(ending + 7));
        std::swap(*(ending + 1), *(ending + 6));
        std::swap(*(ending + 2), *(ending + 5));
        std::swap(*(ending + 3), *(ending + 4));
    }
